...
import { storeMeta, storeInitial, trLines, trAvail } from '../../data/warehouseData'

function Content({ t, onClose, onConfirm }) {
  const m = storeMeta(t.store)
  const lines = trLines(t)
  const [qtys, setQtys] = useState(() => lines.map((l) => Math.min(l.qty, trAvail(l.sku))))
      ...
      <div className="px-6 py-5 space-y-4">
            ...
            {lines.map((l, i) => {
              ...
              return (
                <div key={i} className="grid items-center gap-2 px-3 py-2 bg-gray-50 border border-border rounded-lg" style={{ gridTemplateColumns: '1.7fr 0.6fr 0.6fr 0.7fr' }}>
                  ...
                  <p className={`text-[11px] text-right ${short ? 'text-brand-red font-bold' : 'text-gray-500'}`}>{avail}</p>
        ...
        {partial && (
          <div className="flex items-start gap-2 px-3 py-2.5 bg-brand-orange/5 border border-brand-orange/20 rounded-xl">
            ...
            <p className="text-[11px] text-gray-600 leading-relaxed"><strong className="text-navy-dark">Partial fulfilment.</strong> You're sending fewer units than requested. The store will be notified of the shortfall so they can source the rest elsewhere.</p>
          </div>
        )}
        ...
        <div className="flex items-start gap-2 px-3 py-2.5 bg-brand-orange/5 border border-brand-orange/20 rounded-xl">
          ...
          <p className="text-[11px] text-gray-600 leading-relaxed">Dispatching will <strong className="text-navy-dark">deduct stock</strong> from the warehouse immediately and mark the transfer in transit. This cannot be undone.</p>
        </div>
      </div>
...
export default function DispatchModal({ transfer, onClose, onConfirm }) {
  return <Modal item={transfer} onClose={onClose} render={(t) => <Content t={t} onClose={onClose} onConfirm={onConfirm} />} />
}
