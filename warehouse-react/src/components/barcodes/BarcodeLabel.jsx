  ...
  useEffect(() => {
    if (fmt === 'QR') {
      if (canvasRef.current) {
        QRCode.toCanvas(canvasRef.current, product.sku, { width: Math.min(size.w - 16, 72), margin: 1 }, () => {})
      }
    } else if (svgRef.current) {
      const opts = { width: 1.4, height: Math.round(size.h * 0.42), displayValue: false, margin: 2 }
      try {
        JsBarcode(svgRef.current, product.sku, { ...opts, format: fmt === 'EAN13' ? 'EAN13' : 'CODE128' })
      } catch {
        // EAN13 needs a valid 13-digit numeric code — fall back to Code 128.
        JsBarcode(svgRef.current, product.sku, { ...opts, format: 'CODE128' })
      }
    }
  }, [product.sku, fmt, sizeKey, size.w, size.h])
  ...
  return (
    ...
    >
      <div style={{ textAlign: 'center', width: '100%', overflow: 'hidden' }}>
        ...
        {inclPrice && <p style={{ fontSize: 8, fontWeight: 600, color: '#0a1535', margin: '1px 0 0' }}>Rs.{product.price.toLocaleString()}</p>}
      </div>
      {fmt === 'QR' ? (
      ...
      <p style={{ fontSize: 7, color: '#94a3b8', letterSpacing: '0.05em', margin: '1px 0 0' }}>{product.sku}</p>
    </div>
  )
}
