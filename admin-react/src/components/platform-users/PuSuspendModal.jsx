...
export default function PuSuspendModal({ user, onClose, onConfirm }) {
      ...
      >
        <div className="flex items-center gap-3 px-6 py-5 border-b border-border">
          <div className="w-10 h-10 rounded-xl bg-brand-red/10 flex items-center justify-center shrink-0">
            <Icon name="pause-circle-outline" className="text-brand-red" style={{ fontSize: '20px' }} />
          </div>
          <div>
            <p className="text-[14px] font-bold text-navy-dark">Suspend User Login</p>
            <p className="text-[11px] text-gray-400">Platform-level emergency override.</p>
          </div>
        </div>
        <div className="px-6 py-5">
          <p className="text-[12px] text-gray-600 mb-4">
            You are suspending <strong className="text-navy-dark">{user.name}</strong>'s login. This is for abuse/fraud
            cases only — routine staff management (role changes, offboarding) should be done by the business owner from
            their own portal.
          </p>
          ...
          <textarea
            ...
            placeholder="e.g. Reported for fraudulent transactions."
