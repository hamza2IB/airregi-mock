...
import { SM_FULFILLERS, smSourceStock, storeMeta, storeInitial, SM_STORE_NAME } from '../data/transferData'
...
// Stock at each other location (warehouse + sister branches) the branch can request from.
function otherLocations(row) {
  const reorder = Math.max(4, Math.round((row.reorder || 60) / 3))
  return SM_FULFILLERS.map((st) => {
    const q = smSourceStock(st, row.sku)
...
export default function ScanStock({ onNavigate }) {
  ...
  const branchMeta = storeMeta(SM_STORE_NAME)
    ...
    <div className="p-4 sm:p-6 md:p-8">
      <div className="flex items-start gap-2.5 bg-brand-blue/5 border border-brand-blue/15 rounded-xl px-4 py-3 mb-6">
        ...
        <p className="text-[11px] text-gray-600 leading-relaxed">
          <strong className="text-navy-dark">Scan any product to instantly check its stock.</strong> Use a USB/Bluetooth scanner (it types here automatically), your device camera, or just type a SKU/barcode. You'll see the stock at your branch and how much is available to request from the warehouse and other branches.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-5 items-start max-md:grid-cols-1">
        ...
        <div className="bg-white rounded-xl border border-border overflow-hidden min-h-[200px]">
          <div className="px-5 py-4 border-b border-border">
            ...
            <p className="text-[11px] text-gray-400 mt-0.5">Branch stock &amp; availability to request for the scanned product</p>
          </div>
          <div className="p-5">
            {row ? (
              <>
                ...
                <div className="flex items-center gap-3 p-3 rounded-xl border border-border bg-gray-50 mb-3">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ${branchStatus.c}`}>{branchStatus.l}</span>
                </div>

                {/* Branch stock */}
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.06em] mb-1.5">{SM_STORE_NAME} (This Branch)</p>
                ...
                {/* Available elsewhere to request */}
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.06em] mb-1.5">Available to Request</p>
                <div className="border border-border rounded-xl overflow-hidden mb-4">
                  {otherLocations(row).map((b) => {
                ...
                <button onClick={() => { stopCamera(); onNavigate?.('inventory') }} className="w-full h-9 text-[12px] font-semibold text-white bg-navy rounded-xl hover:bg-navy-light transition flex items-center justify-center gap-1.5">
                  <Icon name="cube-outline" size={15} />View in Stock Levels
                </button>
                <button onClick={() => { stopCamera(); onNavigate?.('requests') }} className="w-full mt-2 h-9 text-[12px] font-semibold text-navy border border-border rounded-xl hover:bg-gray-50 transition flex items-center justify-center gap-1.5">
                  <Icon name="paper-plane-outline" size={15} />Request Stock
                </button>
              ...
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="text-[11px] text-gray-400 mt-0.5 max-w-[260px]">Scan or enter a product to see its stock at your branch and where to request more.</p>
              </div>
            )}
          </div>
        </div>
