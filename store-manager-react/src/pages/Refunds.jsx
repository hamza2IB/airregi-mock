...
export default function Refunds({ orders, patchRefund }) {
  ...
  const pendingAmount = pending.reduce((s, r) => s + r.refund.amount, 0)
  const processedAmount = processed.reduce((s, r) => s + r.refund.amount, 0)
  ...
  const processRefund = (state, refId) => {
    patchRefund(state.order.id, state.idx, { status: 'processed', refId, processedDate: 'Now' })
    setProcessFor(null)
    showToast(`Refund of ${rs(state.refund.amount)} marked as processed for ${state.order.id}.`, 'success')
  }
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <Kpi icon="time-outline" iconBg="bg-brand-orange/10" iconColor="text-brand-orange" value={pending.length} valueCls="text-brand-orange" label="Pending Refunds" />
        <Kpi icon="cash-outline" iconBg="bg-brand-red/10" iconColor="text-brand-red" value={rs(pendingAmount)} valueCls="text-brand-red" label="Amount Owed to Customers" />
        <Kpi icon="checkmark-circle-outline" iconBg="bg-brand-green/10" iconColor="text-brand-green" value={processed.length} valueCls="text-brand-green" label="Processed" />
        <Kpi icon="layers-outline" iconBg="bg-navy/10" iconColor="text-navy" value={rs(processedAmount)} valueCls="text-navy-dark" label="Total Refunded" />
      </div>

      {/* Info banner */}
      <div className="flex items-start gap-2.5 bg-brand-orange/5 border border-brand-orange/20 rounded-xl px-4 py-3 mb-6">
        <Icon name="information-circle-outline" style={{ fontSize: '15px', color: '#ff9800', flexShrink: 0, marginTop: '1px' }} />
        <p className="text-[11px] text-gray-600 leading-relaxed">Refunds are created automatically when an online order paid via <strong className="text-navy-dark">Bank Transfer / JazzCash / EasyPaisa</strong> is partially accepted due to stock shortages. Process each refund and record a reference number once the money has been returned to the customer.</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-border overflow-hidden">
        <div className="flex items-center gap-3 px-5 py-4 border-b border-border flex-wrap">
          <div className="flex items-center gap-2 bg-page rounded-lg px-3 py-2 border border-border flex-1 min-w-[200px] max-w-xs">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} type="text" placeholder="Search order #, customer…" className="bg-transparent text-[12px] text-navy-dark placeholder-gray-400 flex-1 border-none outline-none" />
          </div>
        ...
        <div className="overflow-x-auto thin-scroll">
          <div className="min-w-[820px]">
            <div className="grid text-[10px] font-semibold text-gray-400 uppercase tracking-[0.08em] px-5 py-2.5 border-b border-border bg-gray-50/60" style={{ gridTemplateColumns: COLS }}>
              <div>Order #</div><div>Customer</div><div>Reason</div>
              <div className="text-center">Amount</div><div>Payment</div>
            ...
            {filtered.length === 0 ? (
              ...
              <div className="divide-y divide-gray-100">
                {filtered.map(({ order: o, refund: r, idx }) => {
                  ...
                  return (
                    <div key={`${o.id}-${idx}`} className="grid items-center px-5 py-3.5 hover:bg-gray-50/50 transition" style={{ gridTemplateColumns: COLS }}>
                      ...
                      <div className="min-w-0">
                        <p className="text-[11px] text-gray-600 truncate">{r.reason}</p>
                        <p className="text-[10px] text-gray-400">{r.items.length} item{r.items.length !== 1 ? 's' : ''} affected</p>
                      </div>
                      <p className="text-[13px] font-bold text-brand-red text-center">{rs(r.amount)}</p>
                      <div className="min-w-0">
                        <p className="text-[11px] font-medium text-navy-dark">{o.payment}</p>
                        <p className="text-[10px] text-gray-400 font-mono truncate">{o.paymentRef || '—'}</p>
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 justify-end">
                        ...
                        {isPending && (
                          <button onClick={() => setProcessFor({ order: o, refund: r, idx })} className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-semibold border border-brand-orange/30 bg-brand-orange/10 text-brand-orange hover:bg-brand-orange/20 transition shrink-0 whitespace-nowrap">
                            <Icon name="checkmark-outline" style={{ fontSize: '12px' }} />Process
                          </button>
                        )}
                      </div>
                    </div>
