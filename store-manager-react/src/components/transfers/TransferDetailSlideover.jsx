...
import { SM_TR_STATUS, SM_STORE_NAME, SM_MANAGER, storeMeta, storeInitial, trLines } from '../../data/transferData'
...
function TrBadge({ status }) {
  const s = SM_TR_STATUS[status] || SM_TR_STATUS.pending
...
function Timeline({ t, inbound }) {
  ...
  const done = () => ['dispatched', 'received'].includes(t.status)
  const steps = [
...
function Content({ t, onClose, showToast, onApprove, onReject, onReceipt, onCancel }) {
  ...
  const routeLine = inbound ? `${t.store} → ${SM_STORE_NAME}` : `${SM_STORE_NAME} → ${counterparty}`
    ...
    <>
      <div className="flex items-center justify-between px-6 py-5 border-b border-border sticky top-0 bg-white z-10">
        <div>
          ...
          <p className="text-[11px] text-gray-400 mt-0.5">{counterparty} · {t.date}</p>
        </div>
      ...
      <div className="p-6">
        ...
        <div className="flex items-center gap-3 p-4 rounded-xl border border-border mb-4" style={{ background: 'rgba(0,0,0,0.015)' }}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-gray-400">
              {inbound ? <>Requested by {t.requestedBy} · Fulfilled by <span className="font-semibold text-gray-500">{SM_STORE_NAME} (you)</span></> : <>Requested by {t.requestedBy} (you) · Fulfilled by <span className="font-semibold text-gray-500">{counterparty}</span></>}
            </p>
          </div>
        ...
        {t.status === 'rejected' && t.rejectReason && (
          <div className="flex items-start gap-2.5 bg-brand-red/5 border border-brand-red/15 rounded-xl px-4 py-3 mb-4">
            ...
            <div>
              <p className="text-[11px] font-bold text-brand-red">Rejected by {t.rejectedBy || (inbound ? SM_MANAGER : counterparty)}</p>
