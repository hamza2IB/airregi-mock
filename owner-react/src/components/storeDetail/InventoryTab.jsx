...
import ProductDetailSlideover from '../products/ProductDetailSlideover'
...
// Store Inventory uses the same product table layout as the Products master, but the
// row actions are stock-focused (View + Update Stock) — this is a specific store's
// on-hand inventory, not the master catalogue, so no add/edit/delete of products.
const PM_PAGE = 8
...
export default function InventoryTab() {
  ...
  // ── Row actions (stock-focused) ──
  const openView = (g) => setDetail(g)
  const openStock = (g) => { setDetail(null); setStockFor(g) }

  // Apply per-SKU on-hand updates to this store's inventory.
  const handleStockSave = (group, updates) => {
    if (updates.length) {
      const bySku = Object.fromEntries(updates.map((u) => [u.sku, u.onHand]))
      setProducts((prev) => prev.map((p) => (p.sku in bySku ? { ...p, onHand: bySku[p.sku] } : p)))
      const units = updates.reduce((s, u) => s + u.onHand, 0)
      showToast(`Stock updated for "${group.name}" — ${updates.length} SKU${updates.length !== 1 ? 's' : ''}, ${units} units on hand.`, 'success')
    }
    setStockFor(null)
  }
  ...
  return (
    <div className="p-5">
              ...
              return (
                <div key={g.name} className="grid items-center px-5 py-3 hover:bg-gray-50/50 transition max-md:grid-cols-[52px_1fr_auto] max-md:gap-2" style={{ gridTemplateColumns: COLS }}>
                  ...
                  <div className="flex items-center justify-center gap-1.5">
                    ...
                    <ActionButton icon="layers-outline" label="Update Stock" onClick={() => openStock(g)} tone="primary" />
                  </div>
                </div>
              )
            })
      ...
      <ProductDetailSlideover item={detail} onClose={() => setDetail(null)} />
      <UpdateStockModal group={stockFor} onClose={() => setStockFor(null)} onSave={handleStockSave} />
    </div>
  )
}
