...
import { INITIAL_STORES, storeAvatarColor } from '../data/storesData'

// Active retail branches the owner oversees (warehouse is shown separately).
const BRANCHES = INITIAL_STORES.filter((s) => s.type === 'Retail' && s.status === 'active')
...
// Deterministic per-branch quantity (mock — owner has no per-store inventory table).
function seededQty(sku, code, base) {
  const s = sku + '|' + code
  let h = 0
  for (let i = 0; i < s.length; i++) h = (h * 31 + s.charCodeAt(i)) & 0x7fffffff
  const cap = Math.max(6, Math.round((base || 60) / 6))
  let q = h % (cap + 1)
  if (h % 11 === 0) q = 0 // sprinkle some genuine zeros
  return q
}
...
function branchStock(row) {
  ...
  return BRANCHES.map((b) => {
    const q = seededQty(row.sku, b.code, row.onHand)
    return { store: b.name, code: b.code, qty: q, reorder, ...ssStatus(q, reorder) }
  })
}
    ...
    <div className="p-8 max-md:p-3.5">
      <div className="flex items-start gap-2.5 bg-brand-blue/5 border border-brand-blue/15 rounded-xl px-4 py-3 mb-6">
        ...
        <p className="text-[11px] text-gray-600 leading-relaxed">
          <strong className="text-navy-dark">Scan any product to see its stock everywhere.</strong> Use a USB/Bluetooth scanner (it types here automatically), your device camera, or type a SKU/barcode. You'll get the <strong className="text-navy-dark">Central Warehouse</strong> count and the live stock across every branch.
        </p>
      </div>
          ...
          <div className="p-5">
            ...
            <div className="mt-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {samples.map((p) => (
                  <button key={p.sku} onClick={() => sample(pmBarcode(p))} className="text-[11px] font-semibold text-navy bg-page border border-border rounded-full px-3 py-1.5 hover:border-navy/40 transition flex items-center gap-1">
              ...
              <>
                ...
                <div className="border border-border rounded-xl overflow-hidden mb-4">
                  {branchStock(row).map((b) => (
                    <div key={b.code} className="flex items-center gap-3 px-3 py-2 border-b border-gray-100 last:border-0">
                      <span className="w-7 h-7 rounded-lg flex items-center justify-center text-[10px] font-extrabold text-white shrink-0" style={{ background: storeAvatarColor(b.store) }}>{b.store.charAt(0).toUpperCase()}</span>
                      <div className="flex-1 min-w-0"><p className="text-[12px] font-semibold text-navy-dark truncate leading-tight">{b.store}</p><p className="text-[10px] text-gray-400 font-mono">{b.code}</p></div>
                ...
                <button onClick={() => { stopCamera(); navigate('/products') }} className="w-full h-9 text-[12px] font-semibold text-white bg-navy rounded-xl hover:bg-navy-light transition flex items-center justify-center gap-1.5">
